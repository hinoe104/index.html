...
function formatTarotVisual(name) {
    let formatted = name.replace(' of ', '<br>of<br>').replace('The ', 'The<br>');
    
    // Handle specific long words manually
    const longWords = {
        "Temperance": "Tempe<br>rance",
        "Judgement": "Judge<br>ment",
        "Hierophant": "Hiero<br>phant",
        "Strength": "Stre<br>ngth",
        "Empress": "Emp<br>ress",
        "Emperor": "Emp<br>eror",
        "Justice": "Jus<br>tice",
        "Hermit": "Her<br>mit",
        "Chariot": "Cha<br>riot",
        "Hanged Man": "Hanged<br>Man"
    };

    Object.keys(longWords).forEach(word => {
        if (formatted.includes(word)) {
            formatted = formatted.replace(word, longWords[word]);
        }
    });
